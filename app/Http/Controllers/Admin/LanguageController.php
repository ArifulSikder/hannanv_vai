...
class LanguageController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        return view('admin.language.manage', compact('language'));
    }
    ...
    public function langUpdate(Request $request, $id)
    {
        ...
        $language->name = $request->name;
        $language->is_default = empty($request->is_default) ? 0 : 1;
        $language->update();
        $notify[] = ['success', 'Update successfully'];
        return redirect()->route('admin.language.index')->withNotify($notify);
    }
